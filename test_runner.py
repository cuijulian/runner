...
def test_should_create_log_file():
    ...
    command_return = subprocess.run("grep data log0.log", shell=True)
...
def test_should_create_sys_trace_logs_when_fail():
    runner.exit_codes = []
    parser = runner.create_parser()
    args = parser.parse_args(['false', '-c', '5', '--failed-count', '1', '--sys-trace'])
    runner.setup_runner(args)
    command_return = subprocess.run("find . -name 'sys_trace_*.log' | wc -l", shell=True, stdout=subprocess.PIPE)
...
def test_should_create_call_trace_logs_when_fail():
    runner.exit_codes = []
    parser = runner.create_parser()
    args = parser.parse_args(['false', '-c', '5', '--failed-count', '3', '--call-trace'])
    runner.setup_runner(args)
    command_return = subprocess.run("find . -name 'call_trace*.log' | wc -l", shell=True, stdout=subprocess.PIPE)
...
def test_should_create_log_trace_logs_when_fail():
    runner.exit_codes = []
    parser = runner.create_parser()
    args = parser.parse_args(['false', '-c', '5', '--failed-count', '2', '--log-trace'])
    runner.setup_runner(args)
    command_return = subprocess.run("find . -name 'log_trace*.log' | wc -l", shell=True, stdout=subprocess.PIPE)
